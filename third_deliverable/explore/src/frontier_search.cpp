...
FrontierSearch::searchFrom(geometry_msgs::msg::Point position)
{
  ...
  // std::vector<unsigned int> new_frontiers = nearestCells(pos,*costmap_);

  // // Print possible frontiers
  // for (unsigned int& value : new_frontiers) {
  //   RCLCPP_INFO(rclcpp::get_logger("FrontierSearch"), "Cell value: %d", value);
  //   }
  ...
  while (!bfs.empty()) {
    ...
    for (unsigned int nbr : nhood8(idx, *costmap_)) {
      ...
      if (isNewFrontierCell(nbr, frontier_flag)) {
        ...
        double distance = trueCost(reference,initial_cell, *costmap_);
        // RCLCPP_INFO(rclcpp::get_logger("FrontierSearch"), "Frontier_dist: %f", distance);
